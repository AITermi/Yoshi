    ...
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[1200px] z-50 bg-slate-100 border-t border-slate-200 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
      <div className="w-full flex flex-col">
        ...
        {!isChatOpen && hasAgreed && (
            <div className="w-full overflow-x-auto no-scrollbar py-2 px-3 flex gap-2 items-center bg-slate-50/50">
                {QUICK_REPLIES.map((reply, index) => (
                    <button
                        key={index}
                        onClick={() => handleQuickReply(reply)}
                        disabled={isLoading}
                        className="shrink-0 bg-white border border-blue-100 text-blue-900 text-[11px] px-3 py-1.5 rounded-full shadow-sm hover:bg-blue-900 hover:text-white hover:border-blue-900 hover:shadow-md transition-all duration-300 animate-in zoom-in-50 slide-in-from-bottom-2 fill-mode-forwards flex items-center gap-1.5"
        ...
        {/* Input Row */}
        <div className="flex items-end gap-2 px-3 pb-2 pt-2 bg-slate-100">
          
          {/* Send Button - Right Side (First in RTL DOM) */}
          <button
            onClick={handleSend}
            disabled={!hasAgreed || !inputText.trim() || isLoading}
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm shrink-0 mb-0.5 ${
              hasAgreed && inputText.trim() && !isLoading
                ? 'bg-blue-900 text-white hover:bg-blue-800 hover:scale-105 active:scale-95' 
                : 'bg-slate-200 text-slate-400 cursor-not-allowed'
            }`}
          >
            <Send size={18} className={isLoading ? 'animate-pulse' : '-ml-0.5 mt-0.5'} strokeWidth={2} />
          </button>
          ...
          <div className="flex-1 relative">
            <input
              ...
              className={`w-full bg-white text-slate-900 px-4 py-2.5 rounded-[20px] focus:outline-none font-light text-right shadow-sm border transition-all text-sm ${
                  hasAgreed 
          ...
          <div className="flex items-center gap-3 pr-1 mb-2.5">
            <a 
